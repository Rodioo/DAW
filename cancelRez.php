        ...
        if($compToken === $token) {
            ...
            if(mysqli_num_rows($result) == 0)
                ...
                if(mysqli_num_rows($result) == 0)
                    ...
                    window.alert('Nu ati facut nicio rezervare pentru aceasta carte.');
                    ...
                    $sql = "update rezervari set status = 'anulata' where isbn = '$isbn' and id_user = '$id_user' and status = 'buna';";
                    mysqli_query($con, $sql);
        
                    if(mysqli_affected_rows($con) == 0)
                        $merge = 0;

                    $sql = "update inchirieri set data_returnare = '2000-1-1' where data_returnare is null and isbn = '$isbn' and id_user = '$id_user';";
                    mysqli_query($con, $sql);
                    ...
                    if($merge == 0)
                        echo ("<script LANGUAGE='JavaScript'>
                        window.alert('Eroare la anulare');
                        window.location.href='mainCont.php';
                        </script>");
                    else
                        echo ("<script LANGUAGE='JavaScript'>
                        window.alert('Rezervare anulata cu succes.');
                        window.location.href='mainCont.php';
                        </script>");
                }
    ...
    <body>
        ...
        <div class="signup-form">
            <form action ="#" method="post">
                ...
                <input type = "submit" value="Anuleaza rezervare" class="registerBtn" name="btn-save">
            </form>
        </div>

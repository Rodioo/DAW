        ...
        if($compToken === $token) {
            ...
            if(mysqli_num_rows($result) == 0)
                ...
                if($nr_ex > 0){
                    ...
                    $sql = "select * from inchirieri where id_user = '$id_user' and isbn = '$isbn' and data_returnare is null";
                    $result = mysqli_query($con, $sql);

                    if(mysqli_num_rows($result) == 0){

                        $next_mth = new DateTime($data_inchiriere);
                        $next_mth = $next_mth->add(new DateInterval('P1M'));
                        $next_mth = $next_mth->format('Y-m-d');
                        
                        $sql2 = "insert into inchirieri (data_inchiriere, id_user, isbn, data_limita) values ('$data_inchiriere', '$id_user', '$isbn', '$next_mth')";
                        ...
                        if($result2){
                            ...
                            window.alert('Inchiriere facuta cu succes');
                            window.location.href='mainCont.php';
                            </script>");
                        }  
                        else
                            echo ("<script LANGUAGE='JavaScript'>
                            window.alert('Eroare la crearea inchirierii.');
                            window.location.href='mainCont.php';
                            </script>");  
                    }
                    else
                        echo ("<script LANGUAGE='JavaScript'>
                        window.alert('Aveti deja aceasta carte inchiriata.');
                        window.location.href='mainCont.php';
                        </script>");
    ...
    <body>
        ...
        <div class="signup-form">
            <form action ="#" method="post">
                ...
                <input type = "submit" value="Inchiriaza" class="registerBtn" name="btn-save">
            </form>
        </div>
